...
The paper proves four results. Here they are in plain English, with what each one *predicts* and (critically) what it *rules out*. Equation numbers point to the [paper PDF](../paper/ICML2026_DeterministicHorizon_FINAL.pdf).

## Theorem 1 — Context-dependent per-step error

$$ \varepsilon(d) \;=\; \varepsilon_0 \;+\; \gamma \cdot \frac{d}{L} $$

The probability of corrupting an intermediate state grows linearly with how deep you are into the chain. Two named constants: $\varepsilon_0$ (baseline noise per step) and $\gamma$ (context-dilution slope). Integrating gives the closed-form *super-exponential* decay

$$ P(\text{correct at depth }d) \;\approx\; \exp\!\left(-d\varepsilon_0 - \frac{\gamma\, d(d+1)}{2L}\right). $$
...
The number of distinct states a decoder-only transformer can reliably track is bounded by a function of heads ($H$), context length ($L$), and per-head dimension ($d_h$). And the bound is *tight* — there exist tasks that saturate it.
...
$$ d^\star \;\approx\; \frac{1}{\gamma}\!\left(\sqrt{2L\ln(1/\alpha)} \;-\; \varepsilon_0 L\right) $$
